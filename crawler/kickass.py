...
try:
  ...
  r = s.get( kat_link, headers=headers, verify=False )
  data = r.content
...
for c in parsed.findAll('a', href=True):
  ...
  # Found a magnet link on kickass
  if "magnet:?" in link:
    magnets.append( link )
...
# go through the last 400 pages in new
i=2
while i<=400:
  ...
  try:
    ...
    r = s.get( kat_link + "/" + str(i), headers=headers, verify=False )
    data = r.content
    parsed = BeautifulSoup(data)
  except Exception, e:
    print e
